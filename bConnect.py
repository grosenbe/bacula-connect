#!/usr/bin/env python3
"""Take a job number and list size of all files backed up in that job."""
import psycopg2
import getpass
import argparse
...
def decode_lstats(stats):
    """Decode the lstats field."""
    # taken from
    # https://gist.github.com/Xiol/ee9d6e9d44494ea8df85
...
def Connect():
    """Connect to the database and get job information."""
    ...
    PW = getpass.getpass('Password for user {0}: '.format(args.user))
    ...
    conn = psycopg2.connect(database=args.database,
                            ...
                            host=args.host,
                            port=args.port)
    # This was just a lucky guess about the client encoding
    ...
    if args.summarize:
        print("\nStatistics for job {0}".format(job))
